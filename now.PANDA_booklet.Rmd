...
ls_gp <- lapply(1:length(ls_df), function(i){
	df_res <- ls_df[[i]]
	gp <- df_res %>% ggplot(aes(study,value)) + geom_col(aes(fill=study), alpha=0.8) + theme_classic() + ylab("Fitness score (Z)") + xlab('Study') + guides(fill='none') + scale_y_continuous(breaks=c(-2,-1,0,1,2), limits=c(-2,2.5)) + scale_x_discrete(labels=c("Current","Kotler et al.")) + geom_hline(yintercept=c(0.47,0.86), color=my_colors, linetype='dashed') + scale_fill_manual(values=my_colors) + labs(title=names(ls_df)[i], subtitle='WT level indicated by colored lines')
})
names(ls_gp) <- names(ls_df)

for(i in 1:length(ls_gp)){
	filename <- str_c('S5/S5_',names(ls_gp)[i],'.png')
	ggsave(filename, ls_gp[[i]], device='png', width=3, height=3, dpi=144)
}
